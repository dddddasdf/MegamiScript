...
{
    private const float IncreaseValueOfAttack = 0.2f;        //공격력 증가 버프에 의해 올라가는 대미지 증가율의 수치는 20%
    private const float DecreaseValueOfAttack = 0.2f;        //공격력 감소 버프에 의해 내려가는 대미지 감소율의 수치는 20%
    private const float IncreaseValueOfDefense = 0.2f;        //방어력 증가 버프에 의해 경감하는 대미지의 수치는 20%
    private const float DecreaseValueOfDefense = 0.2f;        //공격력 증가 버프에 의해 증가하는 대미지 증가율의 수치는 20%
    ...
    //    if (Target.ReturnMemberData().ReturnAffinity(UsedSkill.ReturnSkillType()) == SkillAffinities.Weak)
    //    {
    //        //약점시 1.5배
    //        AffinityRevision = 1.5f;
    //    }
    //    else if (Target.ReturnMemberData().ReturnAffinity(UsedSkill.ReturnSkillType()) == SkillAffinities.Resist)
    //    {
    //        //내성시 0.5배
    ...
    //    for (int i = 0; i < User.ReturnNumberOfIncreaseAttack(); i++)
    //    {
    ...
    public void CalculatePhysicGunDamage(SkillDataRec UsedSkill, IOnBattleObject SkillUser, IOnBattleObject Target, out int PhysicDamage)
    {
        ...
        */
        int SkillPower = (int)UsedSkill.ReturnPower();
        int StOfUser = SkillUser.ReturnSt();     //스킬을 사용한 캐릭터의 힘 스탯 받아오기
        int DxOfUser = SkillUser.ReturnDx();     //스킬을 사용한 캐릭터의 기 스탯 받아오기
        float RoughDamage;          //초벌 계산값 (최종 대미지는 소수점 이하를 잘라낸다)

        float AffinityRevision;     //약점 보정값
        if (Target.ReturnAffinity(UsedSkill.ReturnSkillType()) == SkillAffinities.Weak)
        {
            //약점시 1.5배
            AffinityRevision = 1.5f;
        }
        else if (Target.ReturnAffinity(UsedSkill.ReturnSkillType()) == SkillAffinities.Resist)
        {
            //내성시 0.5배
            AffinityRevision = 0.5f;
        }
    ...
    /// <param name="SkillUser">사용자</param>
    /// <param name="MagicDamage">적용 대상</param>
    public void CalculateMagicDamage(SkillDataRec UsedSkill, IOnBattleObject SkillUser, IOnBattleObject Target, out int MagicDamage)
    {
        /*
        ...
        */
        int SkillPower = (int)UsedSkill.ReturnPower();
        int MaOfUser = SkillUser.ReturnMa();     //스킬을 사용한 캐릭터의 마 스탯 받아오기
        float RoughDamage;          //초벌 계산값 (최종 대미지는 소수점 이하를 잘라낸다)
